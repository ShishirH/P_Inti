...
//        console.log("%c RangeOutput setValue function. shouldAnimate: " + shouldAnimate + ", refreshCanvas: " + refreshCanvas, "background: #4B0082; color: white;");
        ...
        if (refreshCanvas) {
//            ////canvas.renderAll();
            iVoLVER.util.refreshCanvas();
        }

        var i;
        for (i = 0; i < theOutputPoint.outConnections.length; i++) {
            theOutputPoint.outConnections[i].setValue(theOutputPoint.value, false, shouldAnimate);
        }

//      theOutputPoint.outConnections.forEach(function (outConnector) {
        ...
        theOutputPoint.on({
            'moving': function (options) {
                ...
                if (theOutputPoint.connecting) {
                    ...
                    if (event) {
                        var canvasCoords = getCanvasCoordinates(event);
                        var lastAddedConnector = getLastElementOfArray(theOutputPoint.outConnections);
                    ...
                    if (theGenerator) {
                        ...
                        theOutputPoint.inConnections.forEach(function (inConnector) {
            ...
            'mouseup': function (options) {
                ...
                if (theOutputPoint.connecting) {
                    ...
                    var targetObject = findPotentialDestination(canvasCoords, ['isConnectable', 'isVisualProperty', 'isOperator', 'isFunctionInput', 'isAggregator', 'isVisualValue', 'isMapperInput', 'isVerticalCollection', 'isMark', 'isNumericFunctionInput']);
                    var lastAddedConnector = getLastElementOfArray(theOutputPoint.outConnections);
                    ...
                    if (targetObject) {
                        ...
                        if (targetObject !== this) {
                            ...
                            if (targetObject.isConnectable || targetObject.isVisualProperty || targetObject.isFunctionInput || targetObject.isVisualValue || targetObject.isMapperInput || targetObject.isNumericFunctionInput || targetObject.isOperator) {
                            ...
                            var connector = theOutputPoint.outConnections.pop();
                            if (connector) {
                                connector.contract();
                            }
                        }
...
//                            dataType.bringToFront();
            ...
            'pressed': function (options) {
                ...
                theOutputPoint.outConnections.push(newConnector);
