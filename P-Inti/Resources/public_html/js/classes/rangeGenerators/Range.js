...
//        console.log("otherType: " + otherType);
        ...
        if (!otherValue || otherType !== currentType) {
            ...
            otherLimit.inConnections.forEach(function (inConnector) {
                inConnector.contract();
            });
            otherLimit.outConnections.forEach(function (outConnector) {
            ...
            'moving': function (options) {
                ...
                if (!limit.lockMovementX) {
                    ...
                    if (theEvent) {
                        ...
                        var lastAddedConnector = getLastElementOfArray(limit.outConnections);
...
//        console.log("%c computeOutput function NUMBERGENERATOR class. shouldAnimate: " + shouldAnimate, "background: #7FFFD4; color: #000000;");
    ...
    addOutputPoint: function (values, xmlIDs) {
        ...
        var outputPoint = new iVoLVER.model.ValueHolder({
            value: createNumericValue(250),
            path: outputPointPath,
            showLabel: true,
            locked: true
        });
...
function addRange(options) {
    ...
    blink(theGenerator.outputPoint, 0.3);
    blink(theGenerator.lowerLimit, 0.3);
    blink(theGenerator.upperLimit, 0.3);
    blink(theGenerator.playButton, 0.3);
    blink(theGenerator, 0.2);
}
