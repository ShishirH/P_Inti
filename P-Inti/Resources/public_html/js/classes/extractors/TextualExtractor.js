...
//    type: 'textExtractorExtractor',
    ...
    //        options.lockMovementY = true;
        ...
        if (options.values) {
            ...
            dateAndTimeValue = options.values.area || createDefaultValueByTypeProposition('isDateAndTimeValue');
        } else {
            textValue = createStringValue(options.text || '');
            numberValue = createNumericValue(this.number || 0, null, null, '');
            dateAndTimeValue = createDefaultValueByTypeProposition('isDateAndTimeValue');
        }

        this.specificProperties.push({attribute: "text", readable: true, writable: false, types: ['number'], updatesTo: ['area'], dataTypeProposition: 'isStringValue', value: textValue});
        ...
        this.specificProperties.push({attribute: "dateAndTime", readable: true, writable: false, types: ['number'], updatesTo: ['area'], dataTypeProposition: 'isDateAndTimeValue', value: dateAndTimeValue});
...
//        this.specificProperties.push({attribute: "date", readable: true, writable: false, types: ['number'], updatesTo: [], dataTypeProposition: 'isNumericData'});
        ...
        this.applyDeselectedStyle = function () {
...
//                                    var number = parseInt(cleanedText);
                                    ...
                                    if (isNaN(number)) {
                                        ...
                                        numberVisualProperty.outConnections.forEach(function (outConnector) {
...
//                                     dateAndTimeVisualProperty.outConnections.forEach(function (outConnector) {
//                                     outConnector.contract();
//                                     });
                                     }
