...
var VisualProperty = function () {
    ...
    this.disconnect = function (removeAfterDisconnection) {
        var theVisualProperty = this;
        theVisualProperty.inConnections.forEach(function (inConnection) {
            inConnection.contract(false, false);
        });
        theVisualProperty.outConnections.forEach(function (outConnection) {
            outConnection.contract(true, false);
        });
        var waitingTime = 50;
...
//                var arrayNode = createXMLElement("array"); // TODO: It would be better if the created node is still a value one, but with type array
    ...
    this.setValue = function (value, renderCanvas, shouldAnimate) {
        ...
        if (parentObject && (parentObject.isMark || parentObject.isExtractor || parentObject.isNumericCollectionGenerator)) {
            ...
            theVisualProperty.outConnections.forEach(function (outConnector) {
                if (LOG)
                    console.log("The value that will be communicated to the connectors' destinations NNN:");
                if (LOG)
                    console.log(theVisualProperty.value);
                outConnector.setValue(theVisualProperty.value, false, shouldAnimate);
            });
    ...
    this.createValue = function () {
        ...
        if (theVisualProperty.attribute === "fill") {
            return new Value({isColorValue: true, color: new fabric.Color(theVisualProperty.fill)});
    ...
    this.createVisualVisualValue = function () {
        ...
        if (theVisualProperty.isColorValue) {
            options = {
                theType: "color",
                theColor: theVisualProperty.fill
            };

        } else if (theVisualProperty.isStringValue) {
        ...
        } else if (theVisualProperty.isShapeValue) {
    ...
    this.applyDeselectedStyle = function (unselectConnectors) {
        this.selected = false;
        if (this.parentObject && this.parentObject.isMark) {
            this.parentObject.applyDeselectedStyle(false);
        }
...
//                    showLabelModifier(theVisualProperty);
            ...
            'moving': function (options) {
                ...
                if (theEvent) {
                    ...
                    var lastAddedConnector = getLastElementOfArray(this.outConnections);
                ...
                if (targetObject) {
                    ...
                    if (targetObject !== this) {
                        ...
                        if (targetObject.isRangeLimit || targetObject.isOperator || targetObject.isVisualProperty || targetObject.isFunctionInput || targetObject.isVisualValue || targetObject.isFunctionValuesCollection || targetObject.isMapperInput || targetObject.isMapperOutput) {
                            ...
                            var connector = getLastElementOfArray(this.outConnections);
...
//                                            theDestination.bringToFront();
                                    ...
                                    var connector = this.outConnections.pop();
                                    if (connector) {
                                        connector.contract();
                                    }
                    ...
                    var lastAddedConnector = getLastElementOfArray(theVisualProperty.outConnections);
...
//                    alert("The visual property is going to create its own value");
                ...
                this.outConnections.push(newConnector);
            ...
            'newInConnection': function (options) {
                ...
                if (!theVisualProperty.writable) {
                    ...
                    fabric.util.removeFromArray(newInConnection.source.outConnections, newInConnection);
                ...
                if (theVisualProperty.inConnections.length > 0) {
                    var connector = theVisualProperty.inConnections.pop();
                    connector.contract();
                }
...
//                this.inConnections.push(newInConnection);
...
var ReadableVisualProperty = fabric.util.createClass(fabric.Path, {
    ...
    initialize: function (options) {
        ...
        this.setCoords();
        this.id = iVoLVER.util.generateID();
...
function showShapeSelector(theVisualProperty) {
    ...
    var svgFileReadFunction = function (event, file) {
        ...
        fabric.loadSVGFromString(SVGString, function (objects, options) {
            ...
            theVisualProperty.inConnections.forEach(function (inConnector) {
                inConnector.contract();
            });

//            var shapeValue = {isShapeValue: true, value: SVGPATHGROUP_MARK, svgPathGroupMark: svgPathGroupMark};
            var shapeValue = new Value({isShapeValue: true, shape: SVGPATHGROUP_MARK, svgPathGroupMark: svgPathGroupMark});
...
//                        var shapeValue = {isShapeValue: true, value: newShapeType};
                        var shapeValue = new Value({isShapeValue: true, shape: newShapeType, svgPathGroupMark: null});
...
//    var mainDiv = $('<div/>', {class: 'icon-large'});
    ...
    var mainDiv = $('#colorConfigurator_' + holderElement.id + '_' + holderElement.name);

    if (!mainDiv[0]) {

        mainDiv = $('<div/>', {id: 'colorConfigurator_' + holderElement.id + '_' + holderElement.name});
...
//        var colorRectangle = $('<div />', {id: 'colorRectangle_' + holderElement.id + '_' + holderElement.name, style: 'float: left; margin-top: 4px; margin-right: 10px; width: 40px; height: 30px; background: ' + hexColor + '; border-color: #000; border-style: solid; border-width: 1px;'});
        
//        var colorRectangle = $('<input />', {id: 'colorRectangle_' + holderElement.id + '_' + holderElement.name, class: "jscolor {valueElement:null,value:'" + hexColor + "'}", style: 'float: left; margin-top: 4px; margin-right: 10px; width: 40px; height: 30px; border-color: #000; border-style: solid; border-width: 1px;'});

        console.log("ID:");
        console.log('colorRectangle_' + holderElement.id + '_' + holderElement.name);

        var colorRectangle = $('<input />', {id: 'colorRectangle_' + holderElement.id + '_' + holderElement.name, class: 'jscolor', value:'66ccff', style: 'float: left; margin-top: 4px; margin-right: 10px; width: 40px; height: 30px; border-color: #000; border-style: solid; border-width: 1px;'});
        ...
        var rTextField = $('<input />', {id: 'rTextField_' + holderElement.id + '_' + holderElement.name, maxlength: 3, type: 'number', style: 'margin-top: 2px; font-size: 18px; width: 55px; margin-right: 10px;', value: rValue});
        var gTextField = $('<input />', {id: 'gTextField_' + holderElement.id + '_' + holderElement.name, maxlength: 3, type: 'number', style: 'margin-top: 2px; font-size: 18px; width: 55px; margin-right: 10px;', value: gValue});
        var bTextField = $('<input />', {id: 'bTextField_' + holderElement.id + '_' + holderElement.name, maxlength: 3, type: 'number', style: 'margin-top: 2px; font-size: 18px; width: 55px; margin-right: 5px;', value: bValue});

        var colorCanvasWidth = 360;
        var colorCanvasHeight = 200;

        var colorChooserCanvas = $('<canvas />', {id: 'colorChooserCanvas_'+ holderElement.id + '_' + holderElement.name, style: 'margin-top: -5px; width: ' + colorCanvasWidth + 'px; height: ' + colorCanvasHeight + 'px; background-color: #fff; border-color: #000; border-style: solid; border-width: 1px;'});
        ...
        var configurationPanel = $('<div/>', {id: 'theConfigurationPanel_' + holderElement.id + '_' + holderElement.name});
        ...
        okButton.click(function () {
            ...
            $('#colorRectangle_' + holderElement.id + '_' + holderElement.name).jscolor.show();
            
            
            
            document.getElementById('colorRectangle_' + holderElement.id + '_' + holderElement.name).jscolor.show();
            return;
            
            
            if (isRGBColor(selectedColor)) {

                var colorValue = new Value({isColorValue: true, color: new fabric.Color(selectedColor)});
                ...
                holderElement.outConnections.forEach(function (outConnector) {
        ...
        function textBoxChanged() {
            var red = $('#rTextField_' + holderElement.id + '_' + holderElement.name).val();
            var green = $('#gTextField_' + holderElement.id + '_' + holderElement.name).val();
            var blue = $('#bTextField_' + holderElement.id + '_' + holderElement.name).val();
            selectedColor = 'rgb(' + red + ',' + green + ',' + blue + ')';
            $('#colorRectangle_' + holderElement.id + '_' + holderElement.name).css("background-color", selectedColor);
        }
        ...
        image.onload = function () {
            var theCanvas = document.getElementById('colorChooserCanvas_' + holderElement.id + '_' + holderElement.name);
            ...
            function mouseMovingFunction(event) {
                ...
                if (mouseDown && mousePos !== null && mousePos.x > 0 && mousePos.x < colorCanvasWidth && mousePos.y > 0 && mousePos.y < colorCanvasHeight) {
                    ...
                    $('#rTextField_' + holderElement.id + '_' + holderElement.name).val(red);
                    $('#gTextField_' + holderElement.id + '_' + holderElement.name).val(green);
                    $('#bTextField_' + holderElement.id + '_' + holderElement.name).val(blue);
