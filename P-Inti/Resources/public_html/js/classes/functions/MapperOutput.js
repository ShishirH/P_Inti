var MapperOutput = fabric.util.createClass(fabric.Path, {
    ...
    setValue: function (value, shouldAnimate) {
        ...
        theMapperOutput.outConnections.forEach(function (outConnector) {
        ...
        this.on({
            'moving': function (options) {
                ...
                if (theEvent) {
                    ...
                    var lastAddedConnector = getLastElementOfArray(theMapperOutput.outConnections);
            ...
            'mousedown': function (options) {
                ...
                theMapperOutput.outConnections.push(newConnector);
                canvas.add(newConnector);
            },
            'mouseup': function (options) {

                var theMapperOutput = this;
                var connector = getLastElementOfArray(theMapperOutput.outConnections);

                if (!theMapperOutput.value) {
                    connector.contract();
                    alertify.error("No value associated to this output point yet", "", 2000);
                    fabric.util.removeFromArray(theMapperOutput.outConnections, connector);
                    return;
                }
...
//                                    connector.destination.bringToFront();
                            ...
                            connector = theMapperOutput.outConnections.pop();
                            if (connector) {
                                connector.contract();
                            }
                        }
...
//
//
//                        console.log("BEFORE: createdCollection.inConnections:");
//                        console.log(createdCollection.inConnections);
//
//                        connector.setDestination(createdCollection, true);
//                        
//                        console.log("AFTER: createdCollection.inConnections:");
//                        console.log(createdCollection.inConnections);
//
//
//
