...
//        if (theFunction.xCoordinates && theFunction.yCoordinates) {
        if (theFunction.coordinatesX && theFunction.coordinatesY) {
            ...
            if (!theFunction.inputPoint.opacity) {
                ...
                blink(theFunction.inputPoint, 0.3);
            ...
            if (!theFunction.outputPoint.opacity) {
                ...
                blink(theFunction.outputPoint, 0.3);
...
//                        if (theFunction.xCoordinates) {
                        ...
                        if (theEvent) {
                            ...
                            var lastAddedConnector = getLastElementOfArray(numericVisualValue.outConnections);
...
//            theFunction[coordinate + 'Coordinates'] = coordinates;
            ...
            if (coordinate === 'y' && (!theFunction.coordinatesX || !theFunction.xValues.inConnections.length)) {
//            if (coordinate === 'y' && (!theFunction.xCoordinates || !theFunction.xValues.inConnections.length)) {
...
//                    var value = createNumericValue(unscaledValue, inPrefix, outPrefix, units);
                    ...
                    var i;
                    for (i = 0; i < inputPoint.outConnections.length; i++) {
                        inputPoint.outConnections[i].setValue(value, false, shouldAnimate);
                    }
...
function addNumericFunction(options) {
    ...
    if (!options.doNotAnimateAtBirth) {
        if (theFunction.functionPath) {
            blink(theFunction.functionPath, 0.3);
        }
        blink(theFunction.xValues, 0.3);
        blink(theFunction.yValues, 0.3);
        blink(theFunction.inputPoint, 0.3);
        blink(theFunction.outputPoint, 0.3);
        blink(theFunction.minX, 0.3);
        blink(theFunction.maxX, 0.3);
        blink(theFunction.minY, 0.3);
        blink(theFunction.maxY, 0.3);
        blink(theFunction, 0.1);
    }
...
//        theFunction.intersectionPoint.bringToFront();
        ...
        blink(theFunction.intersectionPoint, 0.3);
    }
