    ...
    associateEvents: function () {
        ...
        theInputPoint.on({
            'pressed': function (options) {
                ...
                theInputPoint.outConnections.push(newConnector);
            ...
            'mouseup': function (options) {
                ...
                if (theInputPoint.connecting) {
                    ...
                    var connector = getLastElementOfArray(theInputPoint.outConnections);
...
//                                        connector.destination.bringToFront();
                                ...
                                connector = theInputPoint.outConnections.pop();
                                if (connector) {
                                    connector.contract();
                                }
                            }
...
//                theInputPoint.bringToFront();
            ...
            'moving': function (options) {
                ...
                if (theInputPoint.connecting) {
                    ...
                    var lastAddedConnector = getLastElementOfArray(theInputPoint.outConnections);
                    ...
                    if (theFunction) {
                        ...
                        theInputPoint.inConnections.forEach(function (inConnector) {
...
//                    drawRectAt(new fabric.Point(right, theInputPoint.top), 'green');
            ...
            'newInConnection': function (options) {
                ...
                if (theInputPoint.inConnections.length > 0) {
                    var connector = theInputPoint.inConnections.pop();
                    connector.contract();
                }

                theInputPoint.outConnections.forEach(function (outConnector) {
                    outConnector.setValue(incomingValue, false, shouldAnimate);
                });

                theInputPoint.inConnections.push(newInConnection);
                blink(theInputPoint, 0.3);
            ...
            'inConnectionRemoved': function (options) {
                ...
                if (LOG)
                    console.log("Before: ");
                if (LOG)
                    console.log(theInputPoint.inConnections);

                var removedConnection = options.connector;
                fabric.util.removeFromArray(theInputPoint.inConnections, removedConnection);
    ...
    inValueUpdated: function (options) {
        ...
        var i;
        for (i = 0; i < theInputPoint.outConnections.length; i++) {
            theInputPoint.outConnections[i].setValue(updatedValue, false, shouldAnimate);
        }
