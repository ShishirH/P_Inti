...
var Mark = function () {
    ...
    this.coreProperties.push({attribute: "shape", readable: true, writable: true, types: ['string', 'object'], updatesTo: [], dataTypeProposition: 'isShapeValue'});
    this.coreProperties.push({attribute: "fill", readable: true, writable: true, types: ['string'], updatesTo: [], dataTypeProposition: 'isColorValue'});
    this.coreProperties.push({attribute: "label", readable: true, writable: true, types: ['string', 'number'], updatesTo: [], dataTypeProposition: 'isStringValue'});
...
//            }
    ...
    this.animateBirth = function (markAsSelected, finalScaleX, finalScaleY) {
    ...
    this.applySelectedStyle = function (selectConnectors) {
        ...
        if (selectConnectors) {
            ...
            theMark.inConnections.forEach(function (inConnector) {
                inConnector.applySelectedStyle(true, false);
            });

            theMark.visualProperties.forEach(function (visualProperty) {
                if (visualProperty.inConnections.length || visualProperty.outConnections.length) {
    ...
    this.applyDeselectedStyle = function (unSelectConnectors) {
        ...
        if (unSelectConnectors) {
            ...
            theMark.inConnections.forEach(function (inConnector) {
                inConnector.applyDeselectedStyle(true, false);
            });

            theMark.visualProperties.forEach(function (visualProperty) {
                if (visualProperty.inConnections.length || visualProperty.outConnections.length) {
                    visualProperty.applyDeselectedStyle(true);
                }
            });
        }
    };
    ...
    this.remove = function (shouldDelete) {
        ...
        if (shouldDelete) {
            ...
            theMark.xVisualProperty.disconnect(true);
            theMark.yVisualProperty.disconnect(true);
            ...
            for (var i = 0; i < totalVisualProperties; i++) {
                ...
                visualProperty.disconnect(true);
            }
        ...
        iVoLVER.registerUndoRedo({
            undo: function () {
                var restoredMark = iVoLVER.getObject(theMark.id);
                restoredMark.scaleX = 1;
                restoredMark.scaleY = 1;
                canvas.add(restoredMark);
                canvas.add(restoredMark.backgroundRect);
                restoredMark.positionElements();
                ////canvas.renderAll();
            },
            redo: function () {
                iVoLVER.getObject(theMark.id).remove();
            }
        });
    ...
    this.getDefaultModifiableVisualPropertyByType = function (value) {
        ...
        } else if (value.isDateAndTimeValue || value.isDurationValue) {
            return null;
...
//        theMark.changeColors(fillColor, strokeColor);
    ...
    this.animateVisualProperty = function (i, prop, endValue, duration, easing, removeAfterCompletion, activateEventsAfterAnimation, statusForVisualPropertiesEventsAfterAnimation) {
        var theMark = this;
        ...
        fabric.util.animate({
            ...
            onChange: function (value) {
                ...
                if (prop === 'left') {
                    visualProperty.inConnections.forEach(function (inConnector) {
...
//            drawRectAt(new fabric.Point(x,y), generateRandomColor());
            ...
            visualProperty.outConnections.forEach(function (connector) {
                bringToFront(connector);
            });

            visualProperty.inConnections.forEach(function (connector) {
                bringToFront(connector);
            });
        ...
        for (var i = 0; i < theMark.visualProperties.length; i++) {
            ...
            theMark.animateVisualProperty(i, 'top', positions[i].y, duration, easing, false, isTheLastElement, isTheLastElement);
...
//        var duration = 500;
        ...
        for (var i = 0; i < theMark.visualProperties.length; i++) {
            ...
            theMark.animateVisualProperty(i, 'opacity', 0, duration, easing, true, false, false);
            theMark.animateVisualProperty(i, 'scaleX', 0, duration, easing, true, false, false);
            theMark.animateVisualProperty(i, 'scaleY', 0, duration, easing, true, false, false);
            theMark.animateVisualProperty(i, 'left', theMark.left, duration, easing, true, false, false);
            theMark.animateVisualProperty(i, 'top', theMark.top, duration, easing, true, isTheLastElement, false);
        }
    ...
    this.animatePositionProperty = function (property, endValue, duration, easing) {
...
//        if (LOG) console.log(theMark);
        ...
        theCopy.inConnections = new Array();
        theCopy.set('inConnections', new Array());
...
//                }
            ...
            'inConnectionRemoved': function (options) {
                ...
                if (LOG)
                    console.log("Before: ");
                if (LOG)
                    console.log(theMark.inConnections);

                var removedConnection = options.connector;
                fabric.util.removeFromArray(theMark.inConnections, removedConnection);
...
//                        drawRectAt(p2, "green");
                            ...
                            if (!this.currentCopy) {
                                ...
                                this.currentCopy.applyDeselectedStyle(false);
...
function addMarkToCanvas(options) {

    var type = options.type;

    options.id = options.id || iVoLVER.util.generateID();
...
//    }
    if (type === CIRCULAR_MARK) {
        theMark = addCircularMarkToCanvas(options);
    } else if (type === SQUARED_MARK) {
        theMark = addSquaredMarkToCanvas(options);
    } else if (type === RECTANGULAR_MARK) {
        theMark = addRectangularMarkToCanvas(options);
    } else if (type === ELLIPTIC_MARK) {
        theMark = addEllipticMarkToCanvas(options);
    } else if (type === FATFONT_MARK) {
        theMark = addFatFontMarkToCanvas(options);
    } else if (type === PATH_MARK) {
        theMark = addPathMarkToCanvas(options.thePath, options);
    } else if (type === FILLEDPATH_MARK) {
        theMark = addSVGPathMarkToCanvas(options.thePath, options);
    } else if (type === SVGPATHGROUP_MARK) {
        theMark = addSVGPathGroupMarkToCanvas(options.thePaths, options);
    }

    iVoLVER.registerObject(theMark, options.id);

    iVoLVER.registerUndoRedo({
        undo: function () {
            iVoLVER.getObject(options.id).remove(true);
        },
        redo: function () {
            var restoredMark = iVoLVER.getObject(options.id);
            restoredMark.scaleX = 1;
            restoredMark.scaleY = 1;
            canvas.add(restoredMark);
            canvas.add(restoredMark.backgroundRect);
    ...
    console.log("*************************");
    console.log("theMark.id: " + theMark.id);
    console.log("*************************");
...
function copyVisualPropertiesConnectors(fromMark, toMark) {
    ...
    fromMark.visualProperties.forEach(function (sourceVisualProperty) {
        ...
        if (targetVisualProperty) {
            ...
            // At this point, the inConnections and outConnections from the sourceVisualProperty should be copied to the targetVisualProperty

            sourceVisualProperty.inConnections.forEach(function (inConnector) {
                if (LOG)
...
//            inConnector.setDestination(targetVisualProperty, false);
                ...
                targetVisualProperty.inConnections.push(inConnector);
            });

            sourceVisualProperty.outConnections.forEach(function (outConnector) {
                if (LOG)
...
//            outConnector.setSource(targetVisualProperty);
                ...
                targetVisualProperty.outConnections.push(outConnector);
            });
...
//                }
                ...
                if (svgPathGroupMark) {
                    ...
                    newMark = addMarkToCanvas(options);
                }
...
//    }
    ...
    options.type = markType;

    var theMark = addMarkToCanvas(options);
}