...
//            result = a * b;
    ...
    computeOutputValue: function (shouldAnimate) {
        ...
        if (this.inConnections.length) {
            result = this.inConnections[0].value;
            for (var i = 1; i < this.inConnections.length; i++) {
                result = this.multiplyBy(result, this.inConnections[i].value);
            }
        } else {
