...
//            source.typeIcon.bringToFront();
        ...
        theVisualProperty.inConnections.push(newInConnection);
    ...
    applyDeselectedStyle: function () {
        ...
        this.outConnections.forEach(function (outConnector) {
            if (outConnector.destination && !outConnector.destination.isLocator) {
                outConnector.applyDeselectedStyle(false, true);
            } else {
    ...
    positionChild: function (property, value, theMark, shouldAnimate) {
        ...
        if (theLocator.xAxisArrow) {
            theLocator.xAxisArrow.inConnections.forEach(function (inConnector) {
                inConnector.contract();
            });
        }

        if (theLocator.yAxisArrow) {
            theLocator.yAxisArrow.inConnections.forEach(function (inConnector) {
                inConnector.contract();
            });
        }
    ...
    setProperty: function (property, array, theVisualProperty, shouldAnimate) {
        ...
        theLocator.widgets.forEach(function (widget) {
            ...
            if (widget.isMark) {
                ...
                theMark.xVisualProperty.inConnections.forEach(function (inConnector) {
                    inConnector.contract();
                });
                theMark.yVisualProperty.inConnections.forEach(function (inConnector) {
                    inConnector.contract();
                });
...
//        if (LOG) console.log(theLocator.minUntransformedY);
    ...
    expand: function (refreshCanvas) {
        ...
        fabric.util.animate({
            ...
            easing: easing,
            onChange: function (value) {
                theLocator.outConnections.forEach(function (outConnector) {
        ...
        theLocator.widgets.forEach(function (widget) {
            if (widget.isMark) {
                ...
                if (xVisualProperty.canvas) {
                    ...
                    xVisualProperty.inConnections.forEach(function (inConnector) {
                        inConnector.opacity = 0;
                    });
                    xVisualProperty.outConnections.forEach(function (outConnector) {
                ...
                if (yVisualProperty.canvas) {
                    ...
                    yVisualProperty.inConnections.forEach(function (inConnector) {
                        inConnector.opacity = 0;
                    });
                    yVisualProperty.outConnections.forEach(function (outConnector) {
                        outConnector.opacity = 0;
                    });
    ...
    positionVisualPropertiesConnectorsAtTheOrigin: function () {
        ...
        if (theLocator.xAxisArrow) {
            ...
            theLocator.xAxisArrow.outConnections.forEach(function (outConnector) {
        ...
        if (theLocator.yAxisArrow) {
            ...
            theLocator.yAxisArrow.outConnections.forEach(function (outConnector) {
                outConnector.set({x1: theLocator.left, y1: theLocator.top});
            });
        }
        ...
        theLocator.widgets.forEach(function (widget) {
            ...
            if (widget.isMark) {
                ...
                widget.xVisualProperty.inConnections.forEach(function (inConnector) {
                    inConnector.set({x2: theLocator.left, y2: theLocator.top});
                });

                widget.xVisualProperty.outConnections.forEach(function (outConnector) {
                    outConnector.set({x1: theLocator.left, y1: theLocator.top});
                });

                widget.yVisualProperty.inConnections.forEach(function (inConnector) {
                    inConnector.set({x2: theLocator.left, y2: theLocator.top});
                });

                widget.yVisualProperty.outConnections.forEach(function (outConnector) {
                    outConnector.set({x1: theLocator.left, y1: theLocator.top});
                });
...
//        child.bringToFront();
    ...
    remove: function () {
        var theLocator = this;
        theLocator.outConnections.forEach(function (outConnection) {
            outConnection.contract(true, false, fabric.util.ease['easeOutCubic']);
        });
        hideWithAnimation(theLocator, true);
    },
...
//                if (theLocator.moving || theLocator.scaling) {
                    ...
                    if (targetObject && targetObject !== theLocator) {
                        ...
                        if (targetObject.isMark) {
                            ...
                            var connector = getLastElementOfArray(theLocator.outConnections);
                        ...
                        if (theLocator.lockMovementX && theLocator.lockMovementY) {
                            ...
                            var lastAddedConnector = getLastElementOfArray(theLocator.outConnections);
...
//                        connector.remove();
            ...
            'inConnectionRemoved': function (options) {
                ...
                fabric.util.removeFromArray(this.inConnections, removedConnection);
            },
            'outConnectionRemoved': function (options) {
                ...
                fabric.util.removeFromArray(this.outConnections, removedConnection);
            },
            'newInConnection': function (options) {
...
//                drawRectAt(theMark.getCenterPoint(), theMark.fill);
            ...
            'markSelected': function (theMark) {
                ...
                if (!theLocator.isCompressed) {
                    ...
                    theMark.xVisualProperty.outConnections.forEach(function (outConnector) {
                        outConnector.opacity = 1;
                    });
                    theMark.yVisualProperty.inConnections.forEach(function (inConnector) {
                        inConnector.opacity = 1;
                    });
                    theMark.yVisualProperty.outConnections.forEach(function (outConnector) {
                        outConnector.opacity = 1;
                    });
